...
export function generateModule(grammars, config, grammarConfigMap) {
    ...
    node.append('import { ', config.projectName, `AstReflection } from './ast${config.importExtension}';`, NL, 'import { ');
    ...
    node.indent(moduleNode => {
        moduleNode.append('AstReflection: () => new ', config.projectName, 'AstReflection()', NL);
    });
    node.append('};', NL, NL);
    for (let i = 0; i < grammars.length; i++) {
        const grammar = grammars[i];
        if (grammar.name) {
            ...
            node.indent(moduleNode => {
                var _a;
                moduleNode.append('Grammar: () => ', grammar.name, 'Grammar(),', NL, 'LanguageMetaData: () => ', grammar.name, 'LanguageMetaData,', NL, 'parser: {');
                if ((_a = grammarConfig.chevrotainParserConfig) !== null && _a !== void 0 ? _a : parserConfig) {
