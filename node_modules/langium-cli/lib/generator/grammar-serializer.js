...
export function serializeGrammar(services, grammars, config) {
    ...
    if (config.langiumInternal) {
        node.append(`import type { Grammar } from './ast${config.importExtension}';`, NL, `import { loadGrammarFromJson } from '../../utils/grammar-util${config.importExtension}';`);
    }
    else {
        node.append("import type { Grammar } from 'langium';", NL, "import { loadGrammarFromJson } from 'langium';");
    }
    node.append(NL, NL);
    for (let i = 0; i < grammars.length; i++) {
        ...
        if (grammar.name) {
            ...
            node.append('let loaded', grammar.name, 'Grammar: Grammar | undefined;', NL, 'export const ', grammar.name, 'Grammar = (): Grammar => loaded', grammar.name, 'Grammar ?? (loaded', grammar.name, 'Grammar = loadGrammarFromJson(', delimiter, json, delimiter, '));', NL);
