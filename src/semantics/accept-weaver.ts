...
import { AssignationVisitor, BinaryExpressionVisitor, BlockVisitor, CM, ClockNode, ConstInt, ConstantBooleanVisitor, DeplacementVisitor, DistanceCaptor, IfNode, MM, ProcDeclarationVisitor, RepeatVisitor, SpeedNode, ValCallVisitor, VarDeclarationVisitor } from './visitorNode.js';
...
export class RoboMlAcceptWeaver {
   weaveRobot(node : InterfaceAST.Robot, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitRobot(node as unknown as ClassAST.RobotVisitor);}
    }

    weaveProcDeclaration(node : InterfaceAST.ProcDeclaration, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitProcDeclaration(node as unknown as ProcDeclarationVisitor);}
    }

    weaveBlock(node : InterfaceAST.Block, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitBlock(node as unknown as BlockVisitor);}
    }

    weaveIf(node : InterfaceAST.If, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitIf(node as unknown as IfNode);}
    }

    weaveClock(node : InterfaceAST.Clock, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitClock(node as unknown as ClockNode);}
    }

    weaveConstantInt(node : InterfaceAST.ConstantInt, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitConstInt(node as unknown as ConstInt);}
    }

    weaveSpeed(node : InterfaceAST.Speed, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitSpeed(node as unknown as SpeedNode);}
    }

    weaveCM(node : InterfaceAST.CM, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitCM(node as unknown as CM);}
    }

    weaveMM(node : InterfaceAST.MM, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitMM(node as unknown as MM);}
    }

    weaveDistanceCaptor(node : InterfaceAST.DistanceCaptor, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitDistanceCaptor(node as unknown as DistanceCaptor);}
    }

    weaveAssignation(node : InterfaceAST.Assignation, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitAssignation(node as unknown as AssignationVisitor);}
    }

    weaveValCall(node : InterfaceAST.ValCall, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitValCall(node as unknown as ValCallVisitor);}
    }

    weaveVarDeclaration(node : InterfaceAST.VarDeclaration, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitVarDeclaration(node as unknown as VarDeclarationVisitor);}
    }

    weaveConstantBoolean(node : InterfaceAST.ConstantBoolean, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitConstantBoolean(node as unknown as ConstantBooleanVisitor);}
    }

    weaveDeplacement(node : InterfaceAST.Deplacement, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitDeplacement(node as unknown as DeplacementVisitor);}
    }

    weaveRepeat(node : InterfaceAST.Repeat, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitRepeat(node as unknown as RepeatVisitor);}
    }
    
    weaveBinaryExpression(node : InterfaceAST.BinaryExpression, accept : ValidationAcceptor) : void{
        (<any> node).accept = (visitor: InterpreterImplementation) => {return visitor.visitBinaryExpression(node as unknown as BinaryExpressionVisitor)}
    }
